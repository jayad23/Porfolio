  ...
  return (
    <Card className={stylesHeader.container} style={{ overflow: "hidden" }}>
      ...
      <div className={stylesHeader.cover}>
        <div
          className={clx(stylesHeader.proImgContainer, classes.picContainer)}
        >
          <div className={clx(stylesHeader.profImgWrapper)}>
            <img
              className={clx(classes.profile, stylesHeader.profImg)}
          ...
          <div className={stylesHeader.textWrapper}>
            <h2 className={clx(stylesHeader.text, classes.title)}>
              {gitHubData?.name}
            </h2>
            <p className={clx(stylesHeader.text, classes.description)}>
              {gitHubData?.location}
            </p>
          </div>
        </div>
      ...
      <div className={clx(`${"profile-header-nav"}`, classes.cardContainer)}>
        ...
        >
          <Button
            color=""
            className={clx(`${"btn-icon navbar-toggler"}`, classes.subMenu)}
            onClick={toggle}
          >
          ...
          <Collapse isOpen={isOpen} navbar>
            <div className="profile-tabs d-flex justify-content-between flex-wrap mt-1 mt-md-0">
              <Nav className="mb-0" pills>
                <Button
                  color={`${component === "repo" ? "info" : ""}`}
                  onClick={() => handlerActive("repo")}
                >
                  <span className={classes.hidden} style={ColorShifter(skin)}>
                    {langObj?.repos}: {gitHubData.public_repos}
                  </span>
                  <FileText className="d-block d-md-none" size={14} />
                </Button>
                <Button
                  color={`${component === "user" ? "info" : ""}`}
                  onClick={() => handlerActive("user")}
                >
                  <span className={classes.hidden} style={ColorShifter(skin)}>
                    {langObj?.friends}: {gitHubData.followers}
                  </span>
                  <Users className="d-block d-md-none" size={14} />
                </Button>
                <Button
                  color={`${component === "contribute" ? "info" : ""}`}
                  onClick={() => handlerActive("contribute")}
                >
                  <span className={classes.hidden} style={ColorShifter(skin)}>
                    {langObj?.contributions}
                  </span>
                  <Users className="d-block d-md-none" size={14} />
                </Button>
                <button
